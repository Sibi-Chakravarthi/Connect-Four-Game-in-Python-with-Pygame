...
con=mysql.connector.connect(host="localhost",passwd="123456",user="root")
cur=con.cursor()
cur.execute("show databases")
t=cur.fetchall()
if ("connect4",) not in t:
    cur.execute("create database connect4")
    cur.execute("use connect4")
    cur.execute("create table score (player1 VARCHAR(255), player2 VARCHAR(255),total_time VARCHAR(255), winner VARCHAR(255))")
    con.commit()
else:
    cur.execute("use connect4")
...
def insert_score(player1, player2,total_time, winner):
    global con,cur
    cmd = "INSERT INTO score (player1, player2,total_time, winner) VALUES ('{}', '{}', '{}', '{}')".format(player1, player2,total_time, winner)
    try:
        cur.execute(cmd)
        con.commit()
...
def get_leaderboard():
    global cur
    cur.execute("SELECT player1, player2, total_time, winner FROM score ORDER BY total_time ASC LIMIT 10")
    return cur.fetchall()
...
def display_instructions():
    ...
    while True:
        ...
        for i, line in enumerate(instructions):
            text = font.render(line, True,white_colour)  
            screen.blit(text, (screen.get_width()//2-250, 250 + i * 40))  
        pygame.display.flip()  
...
def display_whose_turn(screen, font, turn):
    ...
    turn_rect = turn_surface.get_rect(topleft=(30, 50))  
...
def display_time(screen, font, yellow_time, red_time):
    ...
    time_rect = time_surface.get_rect(topleft=(30, 10))  
    screen.blit(time_surface, time_rect)
